...
use \Core\Controller;
use \App\models\UtileriasNotificaciones AS UtileriasNotificacionesDao;

ini_set('memory_limit', '-1');

class UtileriasNotificacionesLog extends Controller{
...
html;
      ...
      $logs = UtileriasNotificacionesDao::getAllFiltro($where);
      $tabla= '';
...
html;
    ...
    public static function addAccion($accion){
      ...
      UtileriasNotificacionesDao::insert($log);
    }
...
html;
      ...
      if($ids!=''){
        foreach ($ids as $key => $value) {
          $log = UtileriasNotificacionesDao::getById($value);
            $tabla.=<<<html
              <tr class="tr">
...
html;
        ...
        foreach (UtileriasNotificacionesDao::getAllFiltro($where) as $key => $log) {
          $tabla.=<<<html
            <tr style="background-color:#B8B8B8;">
              <td style="height:auto; width: 200px;background-color:#E4E4E4;">{$log['log_id']}</td>
              <td style="height:auto; width: 200px;background-color:#E4E4E4;">{$log['fecha']}</td>
...
html;
    ...
    public function generarExcel(){
      ...
      if($ids!=''){
        foreach ($ids as $key => $value) {
          $empresa = UtileriasNotificacionesDao::getById($value);
        ...
        foreach (UtileriasNotificacionesDao::getAllFiltro($where) as $key => $value) {
          foreach ($nombreCampo as $key => $campo) {
